...
namespace AURankedPlugin.Plugins.LockedGameSettings.EventListeners
{
    ...
    public sealed class LockedGameSettingsListener : IEventListener
    {
        ...
        public LockedGameSettingsListener(ILogger<LockedGameSettingsPlugin> logger, NormalGameOptions options, RoleValues rolevalues)
        {
            _logger = logger;
            _options = options;
        ...
        private async ValueTask SetSettingsAsync(IGame game, IClientPlayer host)
        {
            if (game.Options is NormalGameOptions gameOptions)
            {
                ...
                ApplyGameOptions(GameDataUtils.gameDataMap[game.Code].gameOptions, gameOptions, _rolevalues);
        ...
        {
            if (!GameDataUtils.gameDataMap.ContainsKey(e.Game.Code))
            {
                GameData data = new GameData(_options, e.Game.Code);
                GameDataUtils.gameDataMap.Add(e.Game.Code, data);
            }
        }
        ...
        public void onChangeMap(IPlayerChatEvent e)
        {
            ...
            if (GameDataUtils.gameDataMap.ContainsKey(e.Game.Code))
            {
                var temp = GameDataUtils.gameDataMap[e.Game.Code];
                _options.Map = mapType;
                _logger.LogInformation($"Before change - Game: {e.Game.Code} Map: {GameDataUtils.gameDataMap[e.Game.Code].gameOptions.Map}");
                temp.setOptions(_options);
                _logger.LogInformation($"After change - Game: {e.Game.Code} Map: {GameDataUtils.gameDataMap[e.Game.Code].gameOptions.Map}");
            }
        }
